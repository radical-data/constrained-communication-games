<script lang="ts">
  ...
  function getMode(modesOptions: Mode[], modeNameOption: string | null): Mode {
    ...
    if (
      !selectedMode ||
      modesOptions.filter((m) => m.name === modeNameOption).length !== 1
    ) {
...
{#if partnerLeft}
  <p>Partner left</p>
  <p>
    Click "New chat" or you will be returned to the home page in {$countdown} seconds.
  </p>
{/if}
<form
  action="#"
  on:submit|preventDefault={sendMessage}
  class:disabled={partnerLeft}
>
  <input
    type="text"
